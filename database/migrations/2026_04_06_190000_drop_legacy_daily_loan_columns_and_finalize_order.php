...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (!empty($existingLegacyColumns)) {
            Schema::table(self::TABLE, function ($table) use ($existingLegacyColumns) {
        ...
        $desiredOrder = array_values(array_filter([
            ...
            'updated_at',
            'created_at',
        ], fn ($name) => $columns->has($name)));

        $unexpectedColumns = [];
        foreach ($columns->keys()->all() as $columnName) {
            if (!in_array($columnName, $desiredOrder, true)) {
                $unexpectedColumns[] = $columnName;
            }
        }

        $finalOrder = array_merge($desiredOrder, $unexpectedColumns);

        $clauses = [];
        $previousColumn = null;

        foreach ($finalOrder as $columnName) {
            $column = $columns->get($columnName);
            $position = $previousColumn === null
                ? ' FIRST'
                : ' AFTER `' . $previousColumn . '`';

            $clauses[] = 'MODIFY COLUMN ' . $this->buildColumnDefinition($column) . $position;
    ...
    {
        // Intentionally left blank because dropped duplicate columns should not be recreated automatically.
    }
