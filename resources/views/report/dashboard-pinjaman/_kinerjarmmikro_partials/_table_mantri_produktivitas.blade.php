<div class="rm-mikro-table-wrap">
    <table class="rm-mikro-table" style="min-width: 1500px;">
        <thead>
            <tr>
                <th rowspan="2">No</th><th rowspan="2">BC</th><th rowspan="2">Nama Uker</th><th rowspan="2">Mantri / pn_pengelola1</th><th rowspan="2">Jumlah Mantri</th>
        ...
        <tbody>
            @forelse ($rows as $index => $row)
                <tr>
                    <td class="text-center">{{ $index + 1 }}</td><td class="strong">{{ $row['bc'] }}</td><td>{{ $row['unit'] }}</td><td class="strong">{{ $row['nama_mantri'] ?? $row['pn_pengelola'] ?? '-' }}</td><td class="text-right">{{ $formatAmount($row['jumlah_mantri'] ?? 0) }}</td>
