...
document.addEventListener('DOMContentLoaded', function () {
    ...
    branchSelect.dataset.state='loading';
    unitSelect.dataset.state='idle';
    ...
    function noResultsMessage(select){const state=select.dataset.state||'ready';if(state==='loading')return'Memuat opsi...';if(state==='empty')return'Tidak ada opsi';if(select===unitSelect)return'Pilih branch office';return'Tidak ada opsi'}
    function initMultiSelect(select, placeholder){if(!(window.jQuery&&window.jQuery.fn&&window.jQuery.fn.select2))return;const $select=window.jQuery(select);if($select.data('select2'))$select.select2('destroy');$select.select2({theme:'bootstrap4',width:'100%',placeholder,closeOnSelect:false,allowClear:true,templateResult:buildOptionTemplate,templateSelection:data=>data.text,escapeMarkup:markup=>markup,language:{noResults:()=>noResultsMessage(select)}})}
    ...
    function setSelectOptions(select, items, selectedValues=[], disabled=false){const normalizedItems=(items||[]).map(normalizeOptionItem).filter(item=>item.value);select.innerHTML='';normalizedItems.forEach(item=>{const option=document.createElement('option');option.value=item.value;option.textContent=item.label;option.selected=selectedValues.includes(item.value);select.appendChild(option)});select.disabled=disabled;select.dataset.state=normalizedItems.length===0?(disabled?'idle':'empty'):'ready';select.dataset.selected=JSON.stringify(selectedValues.filter(value=>normalizedItems.some(item=>item.value===String(value))));refreshSelectUi(select)}
    ...
    resetButton.addEventListener('click',function(){periodInput.value=defaultPeriod;branchSelect.dataset.selected='[]';unitSelect.dataset.selected='[]';branchSelect.dataset.state='loading';unitSelect.dataset.state='idle';setSelectOptions(branchSelect,[],[],true);setSelectOptions(unitSelect,[],[],true);resetTableState();loadFilterOptions()});
